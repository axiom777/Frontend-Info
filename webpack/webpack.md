...
```
Я решил пойти немного по другому - первая переменная это env, вторая переменная options  и так уже содержит информацию в каком режиме запущена сборка.
```
 "scripts": {
...
```
Таким образом при продуктовой сборке - а на github мы учитываем путь для абсолютных путей. Это достаточно простой способ. Можно пойти дальше и создать функцию которая будет возвращать значение не только от mode, но и например какой нибудь переменной окружения если используется env.
